...
OptaPlanner は、100% 純粋に Java(TM) で作られており、バージョン 1.6 以上のすべての JVM 上で動作します。他の Java(TM) 技術との xref:#integration[統合は非常に簡単] です。OptaPlanner は、xref:#useWithMavenGradleEtc[Maven Central Repository] で利用できます。
